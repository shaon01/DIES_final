function [rgbim]=lsaltdisp(hsvim,gamma,scaling)
% $Id$
% (C) Josef Bigun
...
%
%THIS IS B. JAEHNES WAY OF DISPLAYING THE ORIENTATION 
...
%with zero representing the color red. Saturation is modulated by hsvim(:,:,2)/hsvim(:,:,3) whereas
%value is modulated by hsvim(:,:,2). The parameter gamma is a positive
%real number that smoothly converges to 0 when hsvim(:,:,3) is less than gamma.
%The scaling is a switch that turns 
%off the normalization performed on mag(hsvim) before the display.
%These parameters  default to gamma=0.1 and scaling='scloff' when both omitted.
